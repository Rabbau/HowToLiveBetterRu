...
**[Открыть страницу поиска](https://rabbau.github.io/HowToLiveBetterRu/)** · [Оглавление](#оглавление) · [Глоссарий](#как-понимать-цифры-глоссарий) · [Записи проверки источников](docs/核实记录/) · [Стоит ли жениться (полный текст)](docs/стоит-ли-жениться.md) · [Аптечка на случай ЧС (полный текст)](docs/аптечка-чс.md) · [Стоит ли останавливаться, если беда с незнакомцем (полный текст)](docs/помощь-незнакомцу.md) · [Какие документы нужны для платформы (полный текст)](docs/документы-платформы.md)
...
>
> **Переведено полностью**: все 31 глава — в [book/](book/), все четыре развёрнутых материала — в [docs/](docs/) (стоит ли жениться, аптечка на случай ЧС, стоит ли останавливаться ради незнакомца, какие документы нужны для платформы). Китайский оригинал сохранён рядом, не поверх: главы — в [book.zh/](book.zh/), те же четыре документа — под китайскими именами файлов там же в [docs/](docs/), полностью китайская версия этой страницы — [README.zh.md](README.zh.md). Записи проверки источников в [docs/核实记录/](docs/核实记录/) и китайские заметки внутри некоторых источников остаются на китайском — это внутренний журнал работы над книгой, не читательский материал. Страница поиска берёт данные из русского текста книги, но кнопки, фильтры и подписи на самой странице пока китайские — перевод интерфейса ещё предстоит.
>
...
> Ниже — переведённые названия и краткое содержание всех 31 главы. Перевод завершён: все главы ссылаются на файлы в [book/](book/); китайский текст глав остаётся в [book/](book/).
...
Внутри каждой главы пункты идут от самой высокой отдачи к самой низкой. Заголовки глав описывают результат, которого глава пытается избежать, а делать пункт или нет — решает заголовок самого пункта. Развёрнутые материалы: [docs/аптечка-чс.md](docs/аптечка-чс.md) (аптечка на случай ЧС), [docs/документы-платформы.md](docs/документы-платформы.md) (документы для платформы), [docs/стоит-ли-жениться.md](docs/стоит-ли-жениться.md) (стоит ли жениться) и [docs/помощь-незнакомцу.md](docs/помощь-незнакомцу.md) (стоит ли останавливаться ради незнакомца). Ход проверки источников по каждому пункту (на китайском) записан в [docs/核实记录](docs/核实记录/).

Файл `index.html` в корне репозитория — это страница поиска: фильтрует пункты по ключевым словам, главам, уровню доказательности и измерениям затрат (деньги, время, сила воли), данные читает прямо из [README.zh.md](README.zh.md) (китайская версия) и файлов в `book/`. Интерфейс страницы пока не переведён.

## Текст книги

Китайский оригинал разбит по главам на 31 файл в [book/](book/). Русский перевод всех 31 главы готов и лежит в [book/](book/) — это перевод, не адаптация: реалии, законы и цифры остаются китайскими, менялся только язык (подробности — в предупреждении в начале страницы). Переходи по названиям глав в оглавлении выше.
